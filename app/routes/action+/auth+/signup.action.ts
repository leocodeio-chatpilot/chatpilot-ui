...
export async function action({ request }: ActionFunctionArgs) {
  ...
  console.log("debug log 0 - signup.action.ts", parsedSignupPayload);
  ...
  console.log("debug log 0 - signup.action.ts", parsedSignupPayload.data);
  const signupReponse = await signup(parsedSignupPayload.data);

  console.log("debug log 0 - signup.action.ts", signupReponse);
  if (!signupReponse.ok) {
    // 409
    ...
    // 500
      ...
      console.log("debug log 0 - signup.action.ts", await signupReponse.text());
      const result: ActionResultError<SignupPayload> = {
        success: false,
        origin: "email",
